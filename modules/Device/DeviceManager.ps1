...
function Start-SQTLogcat {
    ...
    Write-SQTLog "Checking application..."

    $appPid = Get-SQTAppProcessId `
    ...
    $session = New-SQTSession

    $logFile = Join-Path $session "Logcat.txt"

    Write-SQTLog "Clearing previous Logcat..."
    ...
    Write-SQTLog "Log saved successfully." "SUCCESS"

    $open = Read-Host "Open log file? (Y/N)"
